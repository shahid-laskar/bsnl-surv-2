...
export const CameraCreateSchema = z.object({
  ...
  cam_onvif: z.number({ message: "ONVIF port must be a number" }).int().positive().optional(),
  strm_type_id: z.number({ message: "Stream type is required" }).int().positive("Stream type is required"),
  com_id: z.number({ message: "Customer is required" }).int().positive("Customer is required"),
  device_id: z.number({ message: "Device is required" }).int().positive("Device is required"),
